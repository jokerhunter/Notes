...
## docker安装mysql
mysql设置密码和mysql_native_password模式
```shell
ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY 'tqooteMysql0028';
FLUSH PRIVILEGES;
```

### mysql8开启访问权限
[Mysql 8.0.13 开启远程访问权限](https://blog.csdn.net/GentleCP/article/details/87936263)
[Mysql-8 执行授权语句错误：ERROR 1064 (42000)](https://www.cnblogs.com/fangjb/p/13652551.html)

```sql
mysql> CREATE USER 'root'@'%' IDENTIFIED BY 'tqooteMysql0028';
...
########################⽤户进程分配到的内存设置END############################
#在MySQL暂时停⽌响应新请求之前的短时间内多少个请求可以被存在堆栈中
#官⽅建议back_log = 50 + (max_connections / 5),封顶数为900
back_log = 130
############################⽇志设置##########################################
#数据库错误⽇志⽂件
log_error = error.log
...
binlog_format = ROW
#对于binlog_format = ROW模式时，减少记录⽇志的内容，只记录受影响的列
...
#commit事务时,控制redo log buff持久化磁盘的模式默认为1
innodb_flush_log_at_trx_commit = 2
#commit事务时,控制写⼊mysql binlog⽇志的模式默认为0
...
#强所有发⽣的死锁错误信息记录到error.log中，之前通过命令⾏只能查看最近⼀次死锁信息
...
### redis.conf配置
[redis官方配置文件](https://redis.io/docs/manual/config/)
